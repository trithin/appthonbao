...
class TaskItem {
  ...
  DateTime? muteForDate; // tắt nhắc hôm nay
  DateTime? dueDate; // deadline
  TaskPriority priority;
  List<String> tags;
  DateTime? snoozeUntil; // giờ nhắc lại 1 lần trong ngày
  RepeatFreq repeat; // none/daily/workdays/weekly
...
class NotificationService {
  ...
  Future<void> init() async {
    if (kIsWeb) return; // web không dùng local notifications
  ...
  Future<void> scheduleForTaskToday(TaskItem t) async {
    ...
    // Snooze: nếu cùng ngày và sau hiện tại → đặt 1 lần
...
class _HomePageState extends State<HomePage> {
  ...
  List<TaskItem> get _filtered {
    ...
    list.sort((a, b) {
      ...
      if (overdueA != overdueB) return overdueB - overdueA; // overdue trước
      final pri = b.priority.index.compareTo(a.priority.index); // high trước
      if (pri != 0) return pri;
  ...
  void _spawnNextOccurrence(TaskItem t) {
    ...
    switch (t.repeat) {
      ...
      case RepeatFreq.workdays:
        ...
        } while (next.weekday >= DateTime.saturday); // bỏ T7,CN
        break;
  ...
  void _toggleMuteToday(TaskItem t) async {
    ...
    t.snoozeUntil = null; // bỏ snooze nếu mute
    await _persist();
    await _rescheduleToday();
    setState(() {});
  }
